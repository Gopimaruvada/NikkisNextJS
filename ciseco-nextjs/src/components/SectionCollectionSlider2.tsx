    ...
    <div className={clsx(className, 'relative')}>
        ...
        <div className="-ms-5 embla__container sm:-ms-8">
          {collections.map((collection) => (
            <div
              ...
              onClick={() => setShowIframe(true)} // ⬅️ Open iframe on click
            >
              <CollectionCard2 collection={collection} />
            </div>
          ))}

          {/* Last "More collections" card */}
      ...
      {showIframe && (
        <div className="fixed inset-0 z-50 bg-white">
          <iframe
            src="https://mybillbook.in/store/nikkispetworld"
            className="w-full h-full border-none"
          />
          <button
            className="absolute top-4 right-4 bg-black text-white px-4 py-2 rounded z-50"
